...
//	cam.loadCameraPosition();
	...
	timeline.addCurves("particle focal distance", ofRange(0, 1000),7);
	timeline.addCurves("particle focal range", ofRange(0, 1000), 2);
	...
	gui.add(freezeParticles.setup("freeze particles",ofxParameter<bool>()));
	...
	gui.add(particleMaxSize.setup("max particle size", ofxParameter<float>(), .5, 9));
	...
	textureReplacement.loadImage("Colours3.png");
...
bool testApp::loadScene(string takeDirectory){
    if(player.setup(takeDirectory)){
		...
		//meshBuilder.setTexture(textureReplacement);
...
void testApp::updateParticleSystem(){
	...
	if(!drawParticles || freezeParticles){
		return;
	}
...
//	uniform float maxDisance;
	...
	ambientShader.setUniform1f("maxSize", particleMaxSize) ;
...
//	float particleSize = timeline.getValue("particle size");
	...
	textureReplacement.getTextureReference().bind();
	...
	if(drawParticles){
		...
		cloudShader.begin();
		cloudShader.setUniform1f("focalDistance", timeline.getValue("particle focal distance"));
		cloudShader.setUniform1f("focalRange", timeline.getValue("particle focal range"));
		cloudShader.setUniform1f("maxSize", timeline.getValue("particle size max") );
		cloudShader.setUniform1f("minSize", timeline.getValue("particle size min")) ;
	...
	textureReplacement.getTextureReference().unbind();
